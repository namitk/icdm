...
\frametitle{Goal} \vspace{-2mm}
\begin{itemize}
\item \textbf{Accuracy estimation} : Estimate accuracy of a classifier on a large unlabeled dataset based on a small, possibly unrepresentative, labeled set and a human labeler \vspace*{1mm}
\begin{itemize}
...
\item \textbf{Scalable algorithm} : Perform accuracy estimation on unlabeled data so large that it makes even a single sequential scan impractical in an interactive setting
\begin{itemize}
\item \textbf{Results} : Close to exact estimates while reading three orders of magnitude less data
\end{itemize}
\begin{figure}
\begin{center}
\includegraphics[width=0.53\hsize]{figs/allDataWts-crop}
...
\begin{itemize}
\item Applications rely on output of imperfect classifiers deployed on large datasets \\ 
\begin{itemize}
...
\end{itemize}
\item Unlike \citep{bennett10} and \citep{druck11}, instead of fixing a stratification, we learn a new one every time more data gets labeled
...
\begin{center}
\begin{figure}
\includegraphics[width=0.85\hsize]{figs/e1homeground_crop}
...
\begin{figure}
\includegraphics[width=0.38\hsize]{figs/e1tableannote_crop}
\includegraphics[width=0.38\hsize]{figs/e1spam_crop}
\end{figure}
\begin{figure}
\includegraphics[width=0.38\hsize]{figs/e1homepredicted_crop}
\includegraphics[width=0.38\hsize]{figs/e1dna_crop}
