...
\only<1>{
\textbf{Accuracy estimation} : Estimate accuracy of a classifier on a large unlabeled dataset based on a \textcolor{blue}{small, possibly unrepresentative, labeled set} and a human labeler \vspace*{1mm}
\begin{itemize}
\item \textbf{Our Results} : Between 15\% and 62\% relative reduction in error compared to existing approaches \textit{(See Slogistic)}
\end{itemize}
\begin{center}
\begin{figure}
...
\only<2>{
\textbf{Scalable algorithm} : Perform accuracy estimation on unlabeled data so large that it makes \textcolor{blue}{even a single sequential scan impractical} in an interactive setting
\begin{itemize}
\item \textbf{Our Results} : Able to match within 0.5\% of the estimates of methods based on full scan, while sampling (and labeling) just 2.5k instances from indexed unlabeled data
\end{itemize}
\begin{figure}
\begin{center}
